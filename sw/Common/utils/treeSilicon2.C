...
int loadAlignments(Alignment_t &Ali) {
  string line;
...
int fillEvent( vector<float> *vRecord, Si_t &SiEvent) {
 	...
 	SiEvent.nxClusters[0] = 1;
 	SiEvent.nyClusters[0] = 1;
 	SiEvent.nxClusters[1] = 1;
 	SiEvent.nyClusters[1] = 1;
...
int readSievents(Alignment_t Ali) {

  Si_t SiEvent;

  TFile f("treeSilicon.root","recreate");
  ...
  vector <Hit_t> vHits;
  Hit_t   hit;
  Int_t   hitsubdet[nMaxClusters];
  Int_t   hititrack[nMaxClusters];
  Float_t hitxh    [nMaxClusters];
  Float_t hityh    [nMaxClusters];
  Float_t hitzh    [nMaxClusters];
...
//   TFile f2("treeSiliconAligned.root","recreate");
  ...
  string line;
  ifstream myfile ("../../../data/run200126_multi_000001.dat");
  if (myfile.is_open())
    {
...
void treeSilicon2() {
  Alignment_t Ali;
  int exitcode = 0;
  exitcode = loadAlignments(Ali);
  if (exitcode) return;
  exitcode = readSievents(Ali);
  if (exitcode) return;
  //   tree2r();
}
